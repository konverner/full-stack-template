import { AuthorizationProfileService, type UserRead } from "@/client/index.ts";
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				// 1. Get user profile
				const profile =
					await AuthorizationProfileService.readUsersMeAuthMeGet();
				if (profile) {
					...
					<Paper sx={{ p: 2 }}>
						...
						{userProfile ? (
							<>
								...
								<Typography>
									<strong>Role:</strong>{" "}
									{userProfile.is_superuser ? "Superuser" : "User"}
								</Typography>
								<Typography>
									<strong>Joined:</strong> {formatDate(userProfile.created_at)}
								</Typography>
							</>
						) : (
							<Typography>
