...
const EditForm: React.FC<EditFormProps> = ({
	...
	}, [initialValues, currentUser, username]);
	...
	const trimUserData = (data: UserData): UserData => {
		...
		for (const key in trimmed) {
			if (typeof trimmed[key as keyof UserData] === "string") {
				(trimmed as any)[key] = (
					trimmed[key as keyof UserData] as string
				).trim();
			}
		}
		return trimmed;
	...
	const validateFields = (): boolean => {
		...
		if (
			...
			!/^[a-zA-Z0-9_-]{3,50}$/.test(trimmedData.username)
		) {
			errors.username =
		...
		if (
			trimmedData.avatar_url &&
			!/^https?:\/\/.+\..+/.test(trimmedData.avatar_url)
		) {
	...
	const handleSubmit = async (
		...
		if (!currentUser?.is_superuser) {
			dataToSubmit.is_active = undefined;
			dataToSubmit.is_superuser = undefined;
		}
		...
		try {
			const updatedUser =
				await UsersService.updateUserByUsernameApiV1UsersUsernamePut({
					username,
