...
const EditForm: React.FC<EditFormProps> = ({ initialValues = {}, itemId }) => {
	...
	const handleSubmit = async (
		...
		try {
			let updatedItem: any;
			if (itemId) {
				updatedItem = await ItemsService.updateItemByIdApiV1ItemsItemIdPut({
					itemId,
					requestBody: dataToSubmit,
				});
			} else {
				updatedItem = await ItemsService.createItemApiV1ItemsPost({
					requestBody: dataToSubmit,
				});
			}
			...
			setError(
				err.message ||
					`An error occurred while ${itemId ? "updating" : "creating"} the item.`,
			);
			console.error(err);
		} finally {
			setLoading(false);
