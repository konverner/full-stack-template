...
const ItemDetails: React.FC<ItemDetailsProps> = ({
	...
	const canEditOrDelete =
		...
		(currentUser.is_superuser ||
			(item.owner && currentUser.id === item.owner.id));

	const handleDelete = async (): Promise<void> => {
		if (window.confirm("Are you sure you want to delete this item?")) {
			try {
				await ItemsService.deleteItemApiV1ItemsItemIdDelete({
					itemId: item.id,
				});
				if (onDelete) {
					onDelete();
		...
		<Box>
			<Paper elevation={3} sx={{ p: { xs: 2, sm: 4 }, mt: 2 }}>
				<Grid container spacing={2} alignItems="center" sx={{ mb: 2 }}>
					...
					<Grid>
						<Typography
							variant="h4"
							sx={{ fontWeight: 600, fontSize: { xs: 26, sm: 32 } }}
						>
							{item.name}
						</Typography>
					</Grid>
			...
			{item.description && (
				<Box sx={{ mb: 4, mt: 4 }}>
					...
					<Typography
						variant="body1"
						sx={{ color: "text.secondary", fontSize: { xs: 15, sm: 16 } }}
						// biome-ignore lint/security/noDangerouslySetInnerHtml: description might contain HTML
							...
							<Typography variant="body2" sx={{ mb: 1.5 }}>
								...
								>
									{item.website_url.length > 40
										? `${item.website_url.substring(0, 40)}...`
										: item.website_url}
								</Link>
							</Typography>
						)}
					...
					<Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, height: "100%" }}>
						...
						{item.owner?.username && (
							<Typography variant="body2" sx={{ mb: 1.5 }}>
								Created by:{" "}
								<Link
									href={`/users/${item.owner.username}`}
									underline="hover"
									color="primary"
								>
