...
class UserService:
    ...
    def get_user_by_id(self, db: Session, user_id: int, options: Optional[List[Any]] = None) -> Optional[User]:
        query = db.query(User)
        if options:
            for option in options:
                query = query.options(option)
    ...
    def get_user_by_username(self, db: Session, username: str, options: Optional[List[Any]] = None) -> Optional[User]:
        query = db.query(User)
        if options:
            for option in options:
                query = query.options(option)
    ...
    def update_user(self, db: Session, user: User, user_in: user_schemas.UserUpdate) -> User:
    ...
    def update_user_password(self, db: Session, user: User, password_update: user_schemas.UserPasswordUpdate) -> User:
        if not self.verify_password(password_update.current_password, user.password_hash):
            return None
