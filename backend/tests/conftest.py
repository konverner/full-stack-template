...
def test_user_fixture(db_session: Session) -> User:
    user_in = UserCreate(username="testuser_items", email="testuser@items.com", password="password")
...
def client_auth_fixture(client: TestClient, db_session: Session):
    user_in = UserCreate(username="testuser_auth", email="testauth@example.com", password="testpassword")
...
def superuser_client_auth_fixture(client: TestClient, db_session: Session):
    superuser_in = UserCreate(username="superuser_auth", email="superuser@example.com", password="superpassword", is_superuser=True)
...
def test_item_fixture(db_session: Session, client_auth) -> Item:
    ...
    return item_service.create_item(db_session, item_in=ItemCreate(name="API Test Item"), owner_id=user.id)
...
def test_get_user(db_session: Session):
    user_in = UserCreate(username="testgetuser", email="test@get.com", password="password")
...
def test_list_users(db_session: Session):
    user_service.create_user(db_session, UserCreate(username="listuser1", email="list1@user.com", password="password"))
    user_service.create_user(db_session, UserCreate(username="listuser2", email="list2@user.com", password="password"))
...
def test_update_user(db_session: Session):
    user_in = UserCreate(username="updateuser", email="update@user.com", password="password")
    user = user_service.create_user(db_session, user_in)
    user_in_update = UserUpdate(email="updated@example.com")
    updated_user = user_service.update_user(db_session, user=user, user_in=user_in_update)
    assert updated_user.email == "updated@example.com"

def test_delete_user(db_session: Session):
    user_in = UserCreate(username="deleteuser", email="delete@user.com", password="password")
    user = user_service.create_user(db_session, user_in)
    user_id = user.id
...
def test_create_user_api(client: TestClient, client_auth, superuser_client_auth, db_session: Session):
    # Unauthenticated should fail with 401
    response = client.post(
        "/api/v1/users/",
        json={"username": "apiuser", "email": "api@user.com", "password": "longenoughpassword"},
    )
    ...
    response = client_auth_client.post(
        "/api/v1/users/",
        json={"username": "apiuser2", "email": "api2@user.com", "password": "longenoughpassword"},
    )
    ...
    response = superuser_client.post(
        "/api/v1/users/",
        json={"username": "apiuser3", "email": "api3@user.com", "password": "longenoughpassword"},
    )
...
def test_list_users_api(client: TestClient, db_session: Session):
    user_service.create_user(db_session, UserCreate(username="listuser_api", email="list@api.com", password="longenoughpassword"))
    response = client.get("/api/v1/users/")
    assert response.status_code == 200

def test_get_user_by_username_api(client: TestClient, db_session: Session):
    user_in = UserCreate(username="getme", email="getme@example.com", password="password")
    user = user_service.create_user(db_session, user_in)
...
def test_update_user_api(client_auth, superuser_client_auth, db_session: Session):
    ...
    response = superuser_client.put(
        f"/api/v1/users/{test_user.username}", json={"email": "superuserupdate@example.com"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_delete_user_api(superuser_client_auth, db_session: Session):
    user_to_delete = user_service.create_user(db_session, UserCreate(username="deleteme_api", email="deleteme@api.com", password="longenoughpassword"))
