...
def test_list_items(db_session: Session, test_user: User):
    item_service.create_item(db_session, item_in=ItemCreate(name="Item 1"), owner_id=test_user.id)
    item_service.create_item(db_session, item_in=ItemCreate(name="Item 2"), owner_id=test_user.id)
...
def test_update_item(db_session: Session, test_user: User):
    item = item_service.create_item(db_session, item_in=ItemCreate(name="Original Name"), owner_id=test_user.id)
...
def test_delete_item(db_session: Session, test_user: User):
    item = item_service.create_item(db_session, item_in=ItemCreate(name="To Be Deleted"), owner_id=test_user.id)
    item_id = item.id
...
def test_slug_uniqueness(db_session: Session, test_user: User):
    item1 = item_service.create_item(db_session, item_in=ItemCreate(name="Same Name"), owner_id=test_user.id)
    item2 = item_service.create_item(db_session, item_in=ItemCreate(name="Same Name"), owner_id=test_user.id)
...
def test_update_item_api(client_auth, db_session: Session, test_item: Item):
    ...
    if test_item.owner_id != user.id:
        test_item = item_service.create_item(db_session, ItemCreate(name="Updated Item by Auth User"), owner_id=user.id)

    response = client.put(f"/api/v1/items/{test_item.slug}", json={"name": "Updated via API"})
    assert response.status_code == 200
    data = response.json()
...
def test_delete_item_api(client_auth, db_session: Session, test_item: Item):
    ...
    # If not, create a new item owned by this user for the test
    if test_item.owner_id != user.id:
        test_item = item_service.create_item(db_session, ItemCreate(name="Deleted Item by Auth User"), owner_id=user.id)
